    ...
    <div className="px-4 py-6">
      ...
      <Form method="POST">
        ...
        <div className="relative flex flex-col gap-2 mb-5 sm:items-center sm:flex-row">
          ...
          <div className="grow">
            <input type="text" name="address" required defaultValue={address} disabled={isLoadingAddress} className="w-full input" />

            {addressStatus === 'error' && <p className="p-2 mt-2 text-xs text-red-700 bg-red-100 rounded-md">{errorAddress}</p>}
          </div>


          {!position.latitude && !position.longitude && <span className="absolute right-[3px] top-[35px] z-50 md:right-[4px] md:top-[5px] sm:right-[3px] sm:top-[3px]">
        ...
        <div>
          ...
          <Button type='primary' disabled={isSubmitting || isLoadingAddress}>{isSubmitting ? 'Placing order' : `Order for ${formatCurrency(totalPrice)}`}</Button>
        </div>
      </Form>
    </div>
  );
